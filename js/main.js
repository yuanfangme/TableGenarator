let cols, rows, isDraggedBetweenCells = !1, isMouseDown = !1, mouseDownCell, selectedRowspan, selectedColspan;
...
function exportHTML() {
    ...
    if (clearTh) {
        var a = $("tbody").clone(), a = a.html(), a = a.toLowerCase(), a = a.replace(/rowspan="1"/gi, ""),
            a = a.replace(/colspan="1"/gi, ""), a = a.replace(/class="([^"]*)"/gi, ""),
            a = a.replace(/class="([^"]*)"/gi, ""), a = a.replace(/<div contenteditable="true">/gi, ""),
            a = a.replace(/<div contenteditable=true>/gi, ""), a = a.replace(/&nbsp;/gi, ""),
            a = a.replace(/<\/div>/gi, ""), a = a.replace(/\s\s\s/gi, " "), a = a.replace(/\s\s/gi, " "),
            a = a.replace(/<td\s>/gi, "<td>"), a = a.replace(/<tr>\s<td/gi, "<tr><td"), a = a.replace(/> <\/td>/gi,
            "></td>"), a = a.replace(/<\/td>\s</gi, "</td><"), a = a.replace(/<tr>/gi, enter + bs + "<tr>" + enter),
            a = a.replace(/<\/tr>/gi, enter + bs + "</tr>"), a = a.replace(/d><td/gi, "d>" + enter + "<td"),
            a = a.replace(/<td/gi, bs + bs + "<td"), a = a.replace(/<td>\s/gi, "<td>"), a = a.replace(/\s>\s/gi, ">"),
            a = a.replace(/" >/gi, '">');
        $("#export").val("<table>" + a + enter + "</table>")
    } else {
        var h = $("thead").clone(), h = h.html(), h = h.toLowerCase(), h = h.replace(/rowspan="1"/gi, ""),
            h = h.replace(/colspan="1"/gi, ""), h = h.replace(/class="([^"]*)"/gi, ""),
            h = h.replace(/class="([^"]*)"/gi, ""), h = h.replace(/<div contenteditable="true">/gi, ""),
            h = h.replace(/<div contenteditable=true>/gi, ""), h = h.replace(/&nbsp;/gi, ""),
            h = h.replace(/<\/div>/gi, ""), h = h.replace(/\s\s\s/gi, " "), h = h.replace(/\s\s/gi, " "),
            h = h.replace(/<th\s>/gi, "<th>"), h = h.replace(/<tr>\s<th/gi, "<tr><th");
        h = h.replace(/> <\/th>/gi,
        ...
        var a = $("tbody").clone(), a = a.html(), a = a.toLowerCase(), a = a.replace(/rowspan="1"/gi, ""),
            ...
            a = a.replace(/<\/div>/gi, ""), a = a.replace(/\s\s\s/gi, " "), a = a.replace(/\s\s/gi, " "),
            a = a.replace(/<td\s>/gi, "<td>"), a = a.replace(/<tr>\s<td/gi, "<tr><td"), a = a.replace(/> <\/td>/gi,
            "></td>"), a = a.replace(/<\/td>\s</gi, "</td><"),
            a = a.replace(/<tr>/gi, enter + bs + bs + "<tr>" + enter + bs),
            a = a.replace(/<\/tr>/gi, enter + bs + bs + "</tr>"), a = a.replace(/d><td/gi, "d>" + enter + bs + "<td"),
            a = a.replace(/<td/gi, bs + bs + "<td"), a = a.replace(/<td>\s/gi, "<td>"), a = a.replace(/\s>\s/gi, ">"),
            a = a.replace(/" >/gi, '">');
...
$(function () {
    ...
    $("td").live("mousemove", function () {
        ...
        if(isMouseDown && mouseDownCell != this)
        console.log("mousemove",isMouseDown,isDraggedBetweenCells,this)
    });
